...
namespace wxb
{
    public sealed class Crc32
    {
        ...
        {
            crc = To(crc, buffer, offset, count);
        }

        public static uint To(byte[] bytes)
        {
            if (bytes == null)
                return 0;

            uint crc = 0;
            crc = To(crc, bytes, 0, bytes.Length);
            return crc;
        }

        public static uint To(uint crc, byte[] bytes, int offset, int count)
        {
            if (bytes == null)
            {
                throw new ArgumentNullException("buffer");
